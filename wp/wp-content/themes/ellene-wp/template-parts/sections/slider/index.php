...
$hero_slider_raw = cmb2_get_option('ellene-wp_landing_options', 'hero_slider');
...
if (!function_exists('ellene_wp_hero_extract_youtube_id')) {

    function ellene_wp_hero_extract_youtube_id($url) {
...
if (!function_exists('ellene_wp_extract_tiktok_video_id')) {

    function ellene_wp_extract_tiktok_video_id($url) {
...
foreach ($hero_slider_raw as $slide) {
    ...
    if ($slide_type === 'video') {
        ...
        $video_id = ellene_wp_hero_extract_youtube_id($video_url);
    ...
    if ($slide_type === 'tiktok') {
        ...
        $tiktok_video_id = ellene_wp_extract_tiktok_video_id($tiktok_url);
...
$hero_slider_footer_text = trim((string) cmb2_get_option('ellene-wp_landing_options', 'hero_main_title'));
...
(function() {
    ...
    function ensureYouTubeApi(callback) {
        ...
        window.__elleneWpYouTubeReadyCallbacks = window.__elleneWpYouTubeReadyCallbacks || [];

        window.__elleneWpYouTubeReadyCallbacks.push(callback);



        if (window.__elleneWpYouTubeApiLoading) {
        ...
        window.__elleneWpYouTubeApiLoading = true;
        ...
        window.onYouTubeIframeAPIReady = function() {
            ...
            const queue = window.__elleneWpYouTubeReadyCallbacks || [];

            window.__elleneWpYouTubeReadyCallbacks = [];
