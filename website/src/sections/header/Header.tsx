    ...
    <header className="em-header fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-background/30 px-6 py-5 backdrop-blur-sm md:px-12 md:py-7">
      ...
      <nav className="flex items-center gap-6 text-xs uppercase tracking-[0.22em] text-muted-foreground md:gap-10 md:text-[0.78rem]">
        <div className="hidden items-center gap-4 text-primary md:flex [&_svg]:h-5 [&_svg]:w-5">
          <a href={getMayamiStreamUrl("spotify")} aria-label="Spotify" className="transition-colors hover:text-white">
            <SpotifyIcon />
          </a>
          <a href={getMayamiStreamUrl("apple-music")} aria-label="Apple Music" className="transition-colors hover:text-white">
            <AppleMusicIcon />
          </a>
          <a href={getMayamiStreamUrl("youtube-music")} aria-label="YouTube" className="transition-colors hover:text-white">
            <YouTubeIcon />
          </a>
          <a href={getMayamiStreamUrl("deezer")} aria-label="Deezer" className="transition-colors hover:text-white">
            <DeezerIcon />
          </a>
          <a href={getMayamiStreamUrl("amazon-music")} aria-label="Amazon Music" className="transition-colors hover:text-white">
            <AmazonMusicIcon />
          </a>
          <a href={getMayamiStreamUrl("soundcloud")} aria-label="SoundCloud" className="transition-colors hover:text-white">
            <SoundCloudIcon />
          </a>
        </div>
        ...
        <a href={RELEASES_URL} className="em-link text-foreground">
          RELEASES
        </a>
      </nav>
    </header>
