...
- Phase 4 complétée: mode mutualisé concret activé pour `stream` avec champs partagés (`shared_stream_*`, `shared_stream_platforms`) et toggle explicite `shared_stream_enabled` pour basculer proprement entre source locale et source partagée (sans mélange partiel)