<?php
/**
 * Assets + AJAX du sélecteur d'item branché au template (squelette EM-SITE).
 *
 * Styles/script du sélecteur (sélection AJAX de l'instance, aperçu de section,
 * MAJ live des titres) et handler `wp_ajax_em_site_set_instance`.
 *
 * @package em-site
 */
 ...
 */
function em_site_admin_render_rubrique_items_picker_assets(): void
{
    static $done = false;
    ...
    // Styles de rendu front EM-SITE (.em-rubrique…) pour les aperçus de sections.
    ...
    .em-site-instance-picker__mode-title { margin:0 0 8px; font-size:11px; font-weight:700; letter-spacing:.04em; text-transform:uppercase; color:#6b7280; }
    .em-site-instance-picker__mode-switch { display:flex; align-items:center; gap:10px; }
    .em-site-instance-picker__mode-option { display:inline-flex; align-items:center; gap:6px; font-weight:600; color:#1d2327; cursor:pointer; }
    .em-site-instance-picker__mode-locked { margin:0; font-weight:600; color:#1d2327; }
    .em-site-instance-picker__mode-help { margin:8px 0 0; font-size:12px; color:#6b7280; }
    .em-site-instance-picker__multi { margin:0 0 10px; padding:10px; background:#fff; border:1px solid #e6d9dc; border-radius:6px; }
    .em-site-instance-picker__multi-title { margin:0 0 8px; font-size:11px; font-weight:700; letter-spacing:.04em; text-transform:uppercase; color:#6b7280; }
    .em-site-instance-picker__multi-switch { display:flex; align-items:center; gap:10px; }
    .em-site-instance-picker__multi-timer { display:inline-flex; align-items:center; gap:8px; margin-top:8px; font-size:12px; color:#1d2327; }
    .em-site-instance-picker__multi-timer input { width:80px; }
    .em-site-instance-picker__badge--first { background:#e5eef8; color:#0b4f85; }
    .em-site-instance-picker[data-display-mode="multi"] .em-site-instance-picker__single-radio { display:none; }
    .em-site-instance-picker[data-display-mode="single"] .em-site-instance-picker__multi-include,
    .em-site-instance-picker[data-display-mode="single"] .em-site-instance-picker__multi-first { display:none !important; }
    .em-site-instance-picker { margin:0; padding:0; list-style:none; display:flex; flex-direction:column; gap:4px; }
    .em-site-instance-picker__row { display:flex; align-items:center; justify-content:space-between; gap:10px; padding:6px 10px; background:#fff; border:1px solid #e6d9dc; border-radius:6px; }
    .em-site-instance-picker__row:has(input:checked) { border-color:#751820; box-shadow:inset 0 0 0 1px #751820; }
    .em-site-instance-picker__label { display:flex; align-items:center; gap:8px; cursor:pointer; flex:1 1 auto; margin:0; }
    .em-site-instance-picker__name { font-weight:600; }
    .em-site-instance-picker__badge { font-size:11px; font-weight:600; color:#751820; background:#f1e3e5; border-radius:10px; padding:1px 8px; }
    .em-site-instance-picker__actions { display:flex; align-items:center; gap:10px; flex:0 0 auto; }
    .em-site-instance-picker__eye { background:none; border:none; padding:0; margin:0; cursor:pointer; color:#751820; line-height:1; }
    .em-site-instance-picker__eye.is-active { color:#2271b1; }
    .em-site-instance-picker__eye .dashicons { width:18px; height:18px; font-size:18px; }
    .em-site-instance-picker__edit { color:#751820; text-decoration:none; line-height:1; }
    .em-site-instance-picker__edit .dashicons { width:18px; height:18px; font-size:18px; }
    ...
    <script>
    (function () {
        var saveRequestSeq = new window.WeakMap();
        var NONCE = '<?php echo esc_js(wp_create_nonce('em_site_set_instance')); ?>';
        var SAVED = '<?php echo esc_js(__('Section branchée enregistrée.', 'em-site')); ?>';
        ...
        function findItemName(list, itemSlug) {
            ...
            var row = list.querySelector('.em-site-instance-picker__row .em-site-instance-picker__single-radio[value="' + itemSlug + '"]')
                || list.querySelector('.em-site-instance-picker__row input[type="radio"][value="' + itemSlug + '"]');
            if (!row) { return ''; }
            var nameSpan = row.closest('.em-site-instance-picker__label').querySelector('.em-site-instance-picker__name');
        ...
        function reflectSectionTitle(list, itemSlug) {
            ...
            var row = document.querySelector('.em-site-rubriques-admin__list-item[data-module-slug="' + type + '"] .em-site-rubriques-admin__list-label');
            setLeadingText(row, composite);

            var zone = document.querySelector('.em-site-admin-landing-map [data-module-slug="' + type + '"]:not([data-header-part]) .em-site-admin-landing-map__zone-label');
        ...
        function updateSingleBadge(list, itemSlug) {
            ...
            if (!itemSlug) { return; }
            var input = list.querySelector('.em-site-instance-picker__single-radio[value="' + itemSlug + '"]')
                || list.querySelector('input[type="radio"][value="' + itemSlug + '"]');
            if (!input) { return; }
            var nameSpan = input.closest('.em-site-instance-picker__label').querySelector('.em-site-instance-picker__name');
            if (!nameSpan) { return; }
            var badge = document.createElement('span');
            badge.className = 'em-site-instance-picker__badge';
        ...
        function updateStreamMultiBadges(list, firstItem) {
            list.querySelectorAll('.em-site-instance-picker__badge').forEach(function (b) { b.remove(); });
            if (!firstItem) { return; }
            var first = list.querySelector('.em-site-instance-picker__multi-first[value="' + firstItem + '"]');
            if (!first) { return; }
            var nameSpan = first.closest('.em-site-instance-picker__label').querySelector('.em-site-instance-picker__name');
            if (!nameSpan) { return; }
            var badge = document.createElement('span');
            badge.className = 'em-site-instance-picker__badge em-site-instance-picker__badge--first';
            badge.textContent = '<?php echo esc_js(__('Premier item', 'em-site')); ?>';
            nameSpan.insertAdjacentElement('afterend', badge);
        }
        ...
        function currentPickerState(list) {
            var mode = String(list.getAttribute('data-display-mode') || getDisplayModeValue(list) || 'single');
            var state = {
                mode: mode,
                item: String(list.getAttribute('data-current') || '')
            };

            if (isStreamList(list)) {
                state.transitionMode = String(list.getAttribute('data-transition-mode') || 'manual');
                state.transitionTimer = String(list.getAttribute('data-transition-timer') || '6');
                state.firstItem = String(list.getAttribute('data-first-item') || '');
                state.hiddenItems = normalizeHiddenItems(parseHiddenItems(list));
            }

            return state;
        }

        function livePickerState(list) {
            if (!list) {
                return null;
            }

            var mode = String(list.getAttribute('data-live-display-mode') || '').trim();
            var item = String(list.getAttribute('data-live-current') || '').trim();
            var state = {
                mode: mode,
                item: item
            };
            ...
            if (isStreamList(list)) {
                state.transitionMode = String(list.getAttribute('data-live-transition-mode') || 'manual');
                state.transitionTimer = String(list.getAttribute('data-live-transition-timer') || '6');
                state.firstItem = String(list.getAttribute('data-live-first-item') || '');
                state.hiddenItems = normalizeHiddenItems(JSON.parse(list.getAttribute('data-live-hidden-items') || '[]'));
            }

            return state;
        }

        function ensurePickerBaseline(list) {
            if (!list || list.getAttribute('data-baseline-state')) {
                return;
            }
        ...
        function hasPendingPickerChanges(list) {
            ...
            var live = livePickerState(list);
            if (!live) {
                return false;
            }

            return pickerStateKey(currentPickerState(list)) !== pickerStateKey(live);
        }

        function pickerDraftKey(list) {
            var template = list ? String(list.getAttribute('data-template') || '').trim() : '';
            var type = list ? String(list.getAttribute('data-type') || '').trim() : '';
            return ['instance-picker', template, type].filter(Boolean).join(':');
        }

        function livePickerItem(list) {
            if (!list) {
                return '';
            }

            return String(list.getAttribute('data-live-current') || '').trim();
        }

        function isPendingItemChange(list, itemValue) {
            return livePickerItem(list) !== String(itemValue || '').trim();
        }

        function beginSaveRequest(list) {
            var next = (saveRequestSeq.get(list) || 0) + 1;
            saveRequestSeq.set(list, next);
            return next;
        }

        function isLatestSaveRequest(list, seq) {
            return (saveRequestSeq.get(list) || 0) === seq;
        }
        ...
        function syncStreamMultiControls(list, mode) {
            var root = list.closest('.em-site-rubriques-admin__picker-inner');
            if (!root) { return; }
            var type = list.getAttribute('data-type') || '';
            var multiWrap = root.querySelector('[data-em-multi-options]');
            var timerWrap = root.querySelector('[data-em-multi-timer-wrap]');
            var transitionAuto = root.querySelector('input[name="em-site-transition-' + type + '"][value="auto"]');
            var transitionManual = root.querySelector('input[name="em-site-transition-' + type + '"][value="manual"]');
        ...
        document.addEventListener('click', function (e) {
            var eye = e.target.closest('.em-site-instance-picker__eye');
            if (!eye || !window.EmWpSkeletonPreview) { return; }
            var inner = eye.closest('.em-site-rubriques-admin__picker-inner');
            if (!inner) { return; }
            var list = inner.querySelector('.em-site-instance-picker');
            ...
            var source = inner.querySelector('.em-site-instance-picker__preview[data-item="' + item + '"] .em-site-instance-picker__stage');
                window.EmWpSkeletonPreview.showUnique(type, source, eye);

            var aside = document.querySelector('.em-site-rubriques-admin__aside');
        ...
        function showUsedPreview(scope) {
            ...
            var list = root.querySelector('.em-site-instance-picker');
            if (!list) { return; }
            var type = list.getAttribute('data-type') || '';
            ...
            var inner = list.closest('.em-site-rubriques-admin__picker-inner');
        ...
        function handlePickerMounted(event) {
            ...
            if (container) {
                ...
                container.querySelectorAll('.em-site-instance-picker').forEach(function (list) {
                    if (isHeaderManagedList(list)) {
                        return;
                    }

                    notifyDraftChanged(list, list.getAttribute('data-current') || '', hasPendingPickerChanges(list));
                });
            }
        }
        ...
        function notifyDraftChanged(list, itemSlug, forcedHasPending) {
            var type = list ? (list.getAttribute('data-type') || '') : '';
            var currentItem = String(itemSlug || (list ? (list.getAttribute('data-current') || '') : '') || '');
            var hasPendingChanges = typeof forcedHasPending === 'boolean'
                ? forcedHasPending
                : hasPendingPickerChanges(list);
            var draftKey = pickerDraftKey(list);
            ...
            document.dispatchEvent(new window.CustomEvent('emSiteDraftChanged', {
                detail: {
                    source: 'em_site_set_instance',
                    rubriqueSlug: type,
                    itemSlug: currentItem,
                    draftKey: draftKey,
        ...
        function syncUsedPreviewAfterSave(list) {
            if (!list || !window.EmWpSkeletonPreview) {
                return;
            }
        ...
        function getDisplayModeValue(list) {
            ...
            var root = list.closest('.em-site-rubriques-admin__picker-inner');
            if (!root) { return ''; }
            var radio = root.querySelector('.em-site-instance-picker__mode input[type="radio"]:checked');
            return radio ? String(radio.value || '') : '';
        }

        function isHeaderManagedList(list) {
            if (!list) {
                return false;
            }

            var type = String(list.getAttribute('data-type') || '').trim();
            if (type !== 'header') {
                return false;
            }

            return !!list.closest('.em-site-header-picker');
        }
        ...
        function appendStreamPayload(body, list, mode) {
            ...
            var transition = root.querySelector('input[name="em-site-transition-' + type + '"]:checked');
        ...
        function normalizeSavePayload(list, requestedItem, res) {
            var payload = (res && res.data && typeof res.data === 'object') ? res.data : {};
            var mode = String((payload.display_mode || getDisplayModeValue(list) || 'single'));
            var safeRequested = String(requestedItem || '').trim();

            // Fallback robuste: après confirmation modale, on veut refléter l'item
            // demandé immédiatement même si le backend renvoie une charge partielle.
            if (safeRequested !== '' && mode === 'single') {
                payload.item = safeRequested;
            }

            if (!payload.item) {
                payload.item = String(list.getAttribute('data-current') || '');
            }

            if (!payload.display_mode) {
        ...
        function saveInstance(list, itemValue, status) {
            var requestSeq = beginSaveRequest(list);
            var body = new URLSearchParams();
            body.set('action', 'em_site_set_instance');
            ...
            fetch(window.ajaxurl, {
                ...
                if (!isLatestSaveRequest(list, requestSeq)) {
                    return;
                }

                if (res && res.success) {
                    applySavedResponse(list, normalizeSavePayload(list, itemValue, res));
                    syncUsedPreviewAfterSave(list);
                    notifyDraftChanged(list, itemValue, isPendingItemChange(list, itemValue));
        ...
        function saveDisplayMode(list, mode, status) {
            ...
            fetch(window.ajaxurl, {
                ...
                if (res && res.success) {
                    applySavedResponse(list, normalizeSavePayload(list, list.getAttribute('data-current') || '', res));
                    syncUsedPreviewAfterSave(list);
                    notifyDraftChanged(list, list.getAttribute('data-current') || '', hasPendingPickerChanges(list));
                    setStatus(status, '<?php echo esc_js(__('Mode d\'affichage enregistré.', 'em-site')); ?>', '#2f7a37');
                } else {
        ...
        function revertDisplayMode(list) {
            ...
            var root = list.closest('.em-site-rubriques-admin__picker-inner');
            if (!root) { return; }
            var prev = root.querySelector('.em-site-instance-picker__mode input[type="radio"][value="' + current + '"]');
        ...
        function revertSelection(list) {
            ...
            var prev = list.querySelector('.em-site-instance-picker__single-radio[value="' + current + '"]')
                || list.querySelector('input[type="radio"][value="' + current + '"]');
            if (prev) { prev.checked = true; }
        }

        // Changement de section branchée : enregistrement immédiat (sans modale)
        // pour rester cohérent avec l'aperçu avant mise en ligne.
        document.addEventListener('change', function (e) {
            var radio = e.target.closest('.em-site-instance-picker__single-radio');
            if (!radio) { return; }
            var list = radio.closest('.em-site-instance-picker');
            if (!list) { return; }
            if (isHeaderManagedList(list)) { return; }
            if ((list.getAttribute('data-display-mode') || 'single') === 'multi') { return; }

            // Fige l'état initial avant le premier enregistrement AJAX.
            ensurePickerBaseline(list);
            list.setAttribute('data-baseline-item', baselinePickerItem(list));

            var status = list.parentNode.querySelector('.em-site-instance-picker__status');
            saveInstance(list, radio.value || '', status);
        });

        document.addEventListener('change', function (e) {
            var modeRadio = e.target.closest('.em-site-instance-picker__mode input[type="radio"]');
            if (!modeRadio) { return; }
            var root = modeRadio.closest('.em-site-rubriques-admin__picker-inner');
            if (!root) { return; }
            var list = root.querySelector('.em-site-instance-picker');
            if (!list) { return; }
            if (isHeaderManagedList(list)) { return; }
        ...
        document.addEventListener('change', function (e) {
            var control = e.target.closest('.em-site-instance-picker__multi-include, .em-site-instance-picker__multi-first, [data-em-multi-timer-input], [name^="em-site-transition-"]');
            if (!control) { return; }
            var root = control.closest('.em-site-rubriques-admin__picker-inner');
            if (!root) { return; }
            var list = root.querySelector('.em-site-instance-picker');
            if (!list) { return; }
            if (isHeaderManagedList(list)) { return; }
            ...
            if (transitionMode === 'auto') {
                ...
                if (!firstItem) {
                    setStatus(root.querySelector('.em-site-instance-picker__status'), ERR, '#b32d2e');
                    return;
                }
            }

            saveInstance(list, firstItem, root.querySelector('.em-site-instance-picker__status'));
        });

        document.querySelectorAll('.em-site-instance-picker').forEach(function (list) {
            if (isHeaderManagedList(list)) {
                return;
            }
...
/**
 * AJAX : branche un item EM-SITE au template courant (instance template+type).
 */
function em_site_handle_ajax_set_instance(): void
{
    if (!current_user_can('manage_options')) {
        wp_send_json_error(['message' => 'forbidden'], 403);
    }

    check_ajax_referer('em_site_set_instance');
    ...
    if ($template === '' || $type === '' || !em_site_rubrique_type_exists($type)) {
    ...
    em_site_save_instance($template, $type, $instance_data);
...
add_action('wp_ajax_em_site_set_instance', 'em_site_handle_ajax_set_instance');
