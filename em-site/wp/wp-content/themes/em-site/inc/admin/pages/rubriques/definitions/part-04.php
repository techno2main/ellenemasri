<?php
function em_site_admin_rubrique_skeleton_label(string $module_slug): string
{
    $module_slug = sanitize_key($module_slug);

    $static = [
        'top-bar'         => __('TOP-BAR', 'em-site'),
        'header'          => __('HEADER', 'em-site'),
        'stream'          => __('STREAM', 'em-site'),
        'social'          => __('SOCIAL', 'em-site'),
        'video'           => __('VIDEO', 'em-site'),
    ...
    // NE PAS passer par em_site_admin_site_rubrique_all_definitions() ici : cette
    ...
    if (function_exists('em_site_custom_catalog_module')) {
        $module = em_site_custom_catalog_module($module_slug);
    ...
    $definition = em_site_admin_site_rubrique_static_definitions()[$module_slug] ?? null;
 ...
 */
function em_site_admin_rubrique_editing_template_label(): string
{
    $label = function_exists('em_site_get_editing_template_label')
        ? trim((string) em_site_get_editing_template_label())
        : '';
 ...
 */
function em_site_admin_rubrique_editing_page_description_html(string $module_slug): string
{
    $rubrique_label = em_site_admin_rubrique_label($module_slug);
    $template_label = em_site_admin_rubrique_editing_template_label();

    if ($template_label !== '') {
        $template_markup = '<strong class="em-site-hub__template-name">' . esc_html($template_label) . '</strong>';

        return sprintf(
            /* translators: 1: rubrique label, 2: template label markup */
            __('Tu es dans la rubrique %1$s de %2$s.', 'em-site'),
    ...
    return sprintf(
        ...
        esc_html__('Tu es dans la rubrique %s.', 'em-site'),
        esc_html($rubrique_label)
    );
}
 ...
 */
function em_site_admin_rubrique_should_show_nav(string $page_slug = ''): bool
{
 ...
 */
function em_site_admin_rubrique_nav_tab_definitions(): array
{
    $tabs = [];
 ...
 */
function em_site_admin_rubrique_resolve_active_module(string $module_slug = ''): string
{
    $module_slug = sanitize_key($module_slug);
    ...
    foreach (em_site_admin_site_rubrique_definitions() as $slug => $definition) {
        unset($definition);

        if (in_array($page_slug, em_site_admin_rubrique_module_admin_page_slugs($slug), true)) {
            return (string) $slug;
        }
    }
 ...
 */
function em_site_admin_rubrique_enqueue_nav_assets(string $page_slug = ''): void
{
    if (!em_site_admin_rubrique_should_show_nav($page_slug)) {
        return;
    }

    wp_enqueue_style('em-site-admin-module-common');
}

/**
 * Variables CSS inline pour un onglet rubrique (Couleurs Rubrique).
 */
function em_site_admin_rubrique_tab_style_attr(string $module_slug): string
{
    $colors = function_exists('em_site_admin_module_style_colors_for_preview')
