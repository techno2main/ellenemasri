...
function em_wp_stream_option_name(?string $template_slug = null): string
{
    if ($template_slug === null || $template_slug === '') {
        $template_slug = em_wp_stream_resolve_template_slug();
    }
...
function em_wp_stream_resolve_template_slug(?string $preferred = null): string
{
 ...
 */
function em_wp_stream_get_saved_rubrique_options(?string $template_slug = null): array
{
    if ($template_slug === null || $template_slug === '') {
        $template_slug = em_wp_stream_resolve_template_slug();
    }
    ...
    return wp_parse_args(
        em_wp_get_template_rubrique_options('stream', $template_slug),
        em_wp_stream_rubrique_default_options()
    );
}

function em_wp_stream_resolve_catalog_slug(?string $template_slug, array $rubrique): string
{
    $slug = sanitize_key((string) ($rubrique['stream_slug'] ?? ''));

    if ($slug !== '' && function_exists('em_wp_stream_normalize_catalog_slug')) {
        return em_wp_stream_normalize_catalog_slug($slug);
    }
    ...
    return function_exists('em_wp_catalog_default_entry_slug_if_present')
        ? em_wp_catalog_default_entry_slug_if_present(em_wp_stream_catalog_entries())
        : '';
}
 ...
 * @return array<string, mixed>
 */
function em_wp_stream_merge_rubrique_with_catalog(array $rubrique, ?string $template_slug = null): array
{
    if ($template_slug === null || $template_slug === '') {
        $template_slug = em_wp_stream_resolve_template_slug();
    }

    $catalog_slug = em_wp_stream_resolve_catalog_slug($template_slug, $rubrique);
    $catalog = $catalog_slug !== '' && function_exists('em_wp_stream_get_catalog_options')
        ? em_wp_stream_get_catalog_options($catalog_slug)
        : em_wp_stream_catalog_default_options();

    $merged = wp_parse_args($catalog, em_wp_stream_catalog_default_options());
    $merged['enabled'] = !empty($rubrique['enabled']);
    $merged['stream_slug'] = $catalog_slug;
    $merged['platforms'] = em_wp_stream_get_platforms_list($merged);
...
function em_wp_stream_get_options(?string $template_slug = null): array
{
    ...
    $options['stream_slug'] = em_wp_stream_resolve_catalog_slug($template_slug, $options);
...
function em_wp_stream_get_options_for_front(): array
{
    ...
    $rubrique = em_wp_stream_get_saved_rubrique_options($template_slug);

    return em_wp_stream_merge_rubrique_with_catalog($rubrique, $template_slug);
}
