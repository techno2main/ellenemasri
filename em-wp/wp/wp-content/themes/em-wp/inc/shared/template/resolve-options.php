...
function em_wp_template_resolve_option_name(string $rubrique_slug, string $template_slug): string
{
    ...
    return em_wp_template_option_name($rubrique_slug, $template_slug);
}
 ...
 */
function em_wp_admin_rubrique_catalog_pointer_key(string $rubrique_slug): string
{
 ...
 */
function em_wp_admin_rubrique_guess_catalog_entry_for_template(string $rubrique_slug, string $template_slug): string
{
    $rubrique_slug = sanitize_key($rubrique_slug);
    $template_slug = em_wp_template_sanitize_slug($template_slug);
    ...
    $all = function_exists('em_wp_admin_site_rubrique_all_definitions')
    ...
    $entries = function_exists('em_wp_catalog_hub_entries')
        ? em_wp_catalog_hub_entries($catalog_module)
        : [];

    if ($entries === [] && function_exists('em_wp_custom_catalog_entries')) {
        $entries = em_wp_custom_catalog_entries($catalog_module);
    }
 ...
 * @return array<string, mixed>
 */
function em_wp_admin_rubrique_default_template_options(string $rubrique_slug, ?string $template_slug = null): array
{
    $rubrique_slug = sanitize_key($rubrique_slug);
    ...
    if ($rubrique_slug !== ''
        && function_exists('em_wp_admin_rubrique_is_catalog_linked')
        && em_wp_admin_rubrique_is_catalog_linked($rubrique_slug)) {
        $pointer_key = em_wp_admin_rubrique_catalog_pointer_key($rubrique_slug);
        $options[$pointer_key] = em_wp_admin_rubrique_guess_catalog_entry_for_template($rubrique_slug, $template_slug);
    }
 ...
 */
function em_wp_save_template_rubrique_options(string $template_slug, string $rubrique_slug, array $options): bool
{
    $template_slug = em_wp_template_sanitize_slug($template_slug);
    $rubrique_slug = sanitize_key($rubrique_slug);
 ...
 */
function em_wp_template_skeleton_init_rubrique_options(
    ...
    $defaults = function_exists('em_wp_admin_module_default_style_colors')
        ? em_wp_admin_module_default_style_colors($rubrique_slug)
    ...
    $options = em_wp_admin_rubrique_default_template_options($rubrique_slug, $template_slug);
    ...
    return em_wp_save_template_rubrique_options($template_slug, $rubrique_slug, $options);
}
