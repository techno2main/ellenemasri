...
function em_wp_footer_render_admin_page(): void
{
    ...
    ?>
    <div class="wrap em-wp-release-admin em-wp-header-admin em-wp-admin-module em-wp-hub-sommaire" <?php echo em_wp_admin_module_style_data_attributes_for_module('footer', $field, $options); ?> style="<?php echo esc_attr(em_wp_admin_module_style_inline_vars_for_module('footer', $options)); ?>">
        <?php em_wp_admin_render_settings_notices(); ?>
        <?php em_wp_admin_rubrique_render_editing_page_header('footer'); ?>

        <form id="em-wp-release-form" method="post" action="<?php echo esc_url(em_wp_admin_module_form_action(em_wp_footer_page_slug())); ?>">
            <?php
            ...
            <div class="em-wp-admin-module__panels">
                ...
                em_wp_admin_render_module_panel(
                    __('Footer du catalogue', 'em-wp'),
                    'em-wp-release-panel',
                    static function () use ($field, $selected, $choices): void {
                        ?>
                        <p class="description"><?php esc_html_e('Choisis la release du catalogue à afficher dans la rubrique FOOTER de ce template. Édite le contenu dans Catalogues → Footers.', 'em-wp'); ?></p>
                        <?php
