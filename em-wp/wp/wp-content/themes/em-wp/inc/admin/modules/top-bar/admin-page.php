...
function em_wp_top_bar_render_admin_page(): void
{
    ...
    $choices = function_exists('em_wp_top_bar_catalog_choices') ? em_wp_top_bar_catalog_choices() : [];
    $selected = sanitize_key((string) ($options['top_bar_slug'] ?? ''));

    if ($selected !== '' && function_exists('em_wp_top_bar_normalize_catalog_slug')) {
        $selected = em_wp_top_bar_normalize_catalog_slug($selected);
    }

    $style_defaults = em_wp_admin_module_default_style_colors('top-bar');
    $field = em_wp_top_bar_form_option_key();
    ?>
    <div class="wrap em-wp-top-bar-admin em-wp-header-admin em-wp-admin-module em-wp-hub-sommaire" <?php echo em_wp_admin_module_style_data_attributes_for_module('top-bar', $field, $options); ?> style="<?php echo esc_attr(em_wp_admin_module_style_inline_vars_for_module('top-bar', $options)); ?>">
        ...
        <form id="em-wp-top-bar-form" method="post" action="<?php echo esc_url(em_wp_admin_module_form_action(em_wp_top_bar_page_slug())); ?>">
            ...
            <div class="em-wp-top-bar-admin__panels em-wp-admin-module__panels">
                ...
                em_wp_admin_render_module_panel(
                    __('Top-Bar du catalogue', 'em-wp'),
                    'em-wp-top-bar-panel',
                    static function () use ($field, $selected, $choices): void {
                        ?>
                        <p class="description"><?php esc_html_e('Choisis la top-bar du catalogue à afficher dans ce template. Édite le contenu dans Catalogues → Top-Bars.', 'em-wp'); ?></p>
                        <?php
