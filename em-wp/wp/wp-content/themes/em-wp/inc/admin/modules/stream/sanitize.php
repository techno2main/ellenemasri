 ...
 */
function em_wp_stream_sanitize_rubrique_options($input): array
{
    $template_slug = em_wp_stream_resolve_template_slug();
    $existing = em_wp_stream_get_saved_rubrique_options($template_slug);
    ...
    if ($stream_slug !== '' && function_exists('em_wp_stream_normalize_catalog_slug')) {
        $stream_slug = em_wp_stream_normalize_catalog_slug($stream_slug);
    }

    if ($stream_slug !== '' && function_exists('em_wp_stream_catalog_has') && !em_wp_stream_catalog_has($stream_slug)) {
    ...
    if (function_exists('em_wp_admin_sync_rubrique_visibility_from_post')) {
        em_wp_admin_sync_rubrique_visibility_from_post('stream');
    }
 ...
 * @return array<string, mixed>
 */
function em_wp_stream_sanitize_catalog_options($input): array
{
    if (!is_array($input)) {
        return em_wp_stream_catalog_default_options();
    }
 ...
 * @param mixed $input
 */
function em_wp_stream_sanitize_options($input, bool $sync_rubrique = true): array
{
    if ($sync_rubrique) {
        return em_wp_stream_sanitize_rubrique_options($input);
    }

    return em_wp_stream_sanitize_catalog_options($input);
}
